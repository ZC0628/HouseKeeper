...
public class FiletypeUtil {
	...
	/**
	 * 判断文件类型,将文件要使用的图片，名称以及类型存放入数组中返回给调用者
	 */
	public static String[] getFileType(String end){
		//图片和类型 长度为2 —— 存放图片和类型
		String [] iconAndtype = new String[2];
		//如果没找到就默认  iconAndtype[0] = "icon_text_richtext"; iconAndtype[1] = "TYPE_TXT";
		iconAndtype[0] = "icon_text_richtext";
		iconAndtype[1] = "TYPE_TXT";
		
		//循环二维数组   找到就默认循环后的
		for (int x = 0; x < ICON_TYPE_Table.length; x++) {
			for (int y = 0; y < ICON_TYPE_Table[x].length; y++) {
				//后缀名与小的数组第一列进行比较
				if (end.equals(ICON_TYPE_Table[x][0])) {
					iconAndtype[0] = ICON_TYPE_Table[x][1];
					iconAndtype[1] = ICON_TYPE_Table[x][2];
				}
			}
		}
		return iconAndtype;
	...
	/** 文件MIME类型（主要用作打开操作时，指定打开的指定文件对应所属的MIME类型） */
